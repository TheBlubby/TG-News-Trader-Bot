            ...
            {settings.mexcAccounts.map((account, index) => (
              <div key={account.id} className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm relative">
                ...
                <div className="space-y-4 mb-6">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Прокси (опционально)</label>
                    <div className="flex gap-2">
                      <input type="text" value={account.proxyUrl || ""} onChange={e => {
                        const newAccounts = [...settings.mexcAccounts];
                        newAccounts[index].proxyUrl = e.target.value;
                        setSettings({...settings, mexcAccounts: newAccounts});
                      }} className="flex-1 px-3 py-2 bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 font-mono text-sm" placeholder="ip:port:user:pass (или ip:port)" />
                      <button 
                        onClick={async () => {
                          if (!account.proxyUrl) return alert("Введите прокси для проверки");
                          try {
                            const res = await fetch('/api/test-proxy', {
                              method: 'POST',
                              headers: { 'Content-Type': 'application/json' },
                              body: JSON.stringify({ proxyUrl: account.proxyUrl })
                            });
                            const data = await res.json();
                            if (data.success) {
                              alert("✅ Прокси работает успешно!");
                            } else {
                              alert("❌ Ошибка прокси:\n" + data.message);
                            }
                          } catch (e: any) {
                            alert("❌ Ошибка при проверке прокси: " + e.message);
                          }
                        }}
                        className="px-4 py-2 bg-slate-200 hover:bg-slate-300 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 rounded text-sm transition-colors whitespace-nowrap"
                      >
                        Проверить
                      </button>
                    </div>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Telegram Chat ID для этого аккаунта (опционально)</label>
